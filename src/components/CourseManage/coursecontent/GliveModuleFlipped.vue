<template>
  <div>
    ...
    <el-form ref="firstForm" :model="ruleForm" label-width="240px" label-position="left" :inline="true">
      <div class="fromtitle">
        <i class="titicon"></i>
        <span class="tittxt">翻转课堂</span>
      </div>
      <el-form-item label="是否启用翻转课堂？" class="first-form-item">
    ...
    <template v-if="ruleForm.bEnabled=='0'?true:false">
      ...
      <div class="addBox">
        <div @click="addTableData" style="cursor: pointer;display: inline-block;padding: 0 30px;">
          <i class="additem"></i>
          <el-button type="text" class="addBtn">新增翻转课堂</el-button>
        </div>
      </div>
    </template>


    <el-dialog class="addContent tabplane" title="新增翻转课堂" :visible.sync="dialogVisible" @close="cancel('NewTableForm')">
      <el-form :model="NewTableForm" :rules="rules" ref="NewTableForm" label-width="120px">
        <el-form-item label="翻转课堂的名称" prop="name">
          <el-input v-model="NewTableForm.name" auto-complete="off" class="coursetxt" placeholder="请输入复习阶段的名称"></el-input>
        </el-form-item>
        <el-form-item label="翻转课堂的课程大纲" prop="outline">
          <el-select v-model="NewTableForm.outline" placeholder="选择该翻转课堂的课程大纲" style="width: 90%;">
        ...
        <el-form-item label="翻转课堂的启用人群" prop="people">
          <el-input v-model="NewTableForm.people"  auto-complete="off" placeholder="选择该翻转课堂的启用人群" class="coursetxt"></el-input>
        </el-form-item>
