            ...
            if (ret.status == 0) {
                ...
                if (ret.result.welcome_letter_type == 1) {
                    ...
                    this.ruleForm.welcome_letter =
                        ret.result.welcome_letter_template; //默认的选1时，welcome_letter为空，将其设为通用模板
                }
            ...
            let fromData = {
                ...
                brief_introduction:
                    this.editor.getContent() != `<p>${initial_info}</p>`
                        ...
                        : "",
                "course_source_ids[]": this.$store.state.course
                    .resource_intro_id,
                "course_update_ids[]": this.$store.state.course
                    .course_update_ids,
                "course_update_titles[]": this.$store.state.course
                    .course_update_titles,
                "course_update_contents[]": this.$store.state.course
                    .course_update_contents,
                cover: this.$store.state.course.course_cover
            };
            this.btnLoading = true;
            this.$http.SetCourse(url, fromData).then(res => {
