  ...
  <div class="module-edu-content permission-outlinemodule">
    ...
    <div class="edu_table">
      ...
      <div class="edu-pagination">
        <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :page-sizes="[15, 30,50]" :page-size="page_size" :current-page="pagenum" layout="total, sizes, prev, pager, next, jumper" :total="courselinenum">
        </el-pagination>
      </div>
    </div>
      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="班主任" prop="teachid">
          ...
          <el-select v-model="ruleForm.teacher_id" :disabled="!isidentity" @click="handleClkteach" placeholder="请选择班主任"><!--  v-if="!teachtrue"   :disabled="!isidentity"-->
            <el-option v-for="rev in teachrulelist" :key="rev.id" :label="rev.name" :value="rev.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="项目" prop="project_id">
          <el-select v-model="ruleForm.project_id" :disabled="!isproject" @change="checkproject" placeholder="请选择所属项目">
            <el-option v-for="rev in projectlist" :key="rev.project_name" :label="rev.project_name" :value="rev.project_id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="科目" prop="subject_id">
          <el-select v-model="ruleForm.subject_id" :disabled="!issubject" @change="checksubject" placeholder="请选择所属科目">
            <el-option v-for="(com,index) in boxsubject" :label="com.subject_name" :value="com.subject_id" :key="index"></el-option>
          </el-select>
        </el-form-item>
    ...
    <el-dialog title="学员入班" class="tabplane" :visible.sync="dialogFormVisiblePel" :before-close="handleCloseStudent">
      ...
      <div class="peldrop" v-show="module1">
        <div class="pelclass">
          <el-input size="small" icon="search" v-model="inputstudent" placeholder="请输入学员ID / 手机号 / 邮箱 / 昵称 / 姓名" @keyup.native.enter="handleIconClickChrad" :on-icon-click="handleIconClickChrad"></el-input>
        </div>
        ...
        <div class="pageclass" v-if="courselinenumpel > 0">
          <el-pagination @size-change="handleSizeChangepel" @current-change="handleCurrentChangepel" :page-sizes="[10,20,30]" :page-size="page_sizepel" :current-page="pagenumpel" layout="total, sizes, prev, pager, next, jumper" :total="courselinenumpel">
          </el-pagination>
        </div>
      ...
      <div class="rulemodule" v-show="module2">
        <el-input size="small" icon="search" v-model="inputClass" placeholder="请输入班级ID / 名称" @keyup.native.enter="handleIconClickClass" :on-icon-click="handleIconClickClass"></el-input>
        <div class="selmodule">已选择：
          <span v-for="(item,index) in selmodulelist" :key="index">【班级ID：{{item.class_id}}】{{item.class_name}}</span>
        </div>
...
<!-- list  [1,2,3,4].indexOf(1)> -1 slice([1,2,3,4].indexOf(1),1)    .push()-->
        ...
        <div class="pageclass" v-if="courselinenumclass > 0">
          <el-pagination @size-change="handleSizeChangeclass" @current-change="handleCurrentChangeclass" :page-sizes="[10,20,30]" :page-size="page_sizeclass" :current-page="pagenumclass" layout="total, sizes, prev, pager, next, jumper" :total="courselinenumclass">
          </el-pagination>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      async getClassList(){
        ...
        let ret = await this.$http.getClassListout(params);
        this.flag = true;
        if(ret.status == 0){
      ...
      async coursesubmit(min){
        ...
        let ret = await this.$http.addClassList(params);
      ...
      async updateinfo(ruleForm){  // 修改班级
        ...
        let ret = await this.$http.updateinfoClass(this.ruleForm.class_id,{
      ...
      async teachmin(){
        let ret = await this.$http.teachermin();
      ...
      async getcheckcoursemit(query){  // 创建班级时的课程搜索
        ...
        let ret = await this.$http.checkcoursemit(this.checkprojectname,this.checksubjectname,courseinfo);
        if(ret.status == 0){
      ...
      async checkstudent(){
        ...
        let ret = await this.$http.checkstudent(this.pagenumpel,this.page_sizepel,{search_info:this.inputstudent})
        if(ret.status == 0){
      ...
      async checkclass(){
        ...
        let ret = await this.$http.checkclass(this.orginstudent_id,this.pagenumclass,this.page_sizeclass,{search_info:this.inputClass})
        if(ret.status == 0){
      ...
      async orginpel(){
        ...
        if(this.selmodulelist.length > 0){
          ...
          let ret = await this.$http.OrginClassStudent(params);
          if(ret.status == 0){
            this.$message({
              message:'添加成功！',
              type:'success'
