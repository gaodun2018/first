        ...
        <el-col :span="24" class="stu_cou_res_card edbox">
          <el-card :body-style="{ padding: '0px 20px' }" class="plane-card" v-loading="Loading" element-loading-text="拼命加载中...">
            <el-tabs type="card" v-model="activeName" @tab-click="handleClick">
              <el-tab-pane v-for="(res,index) in taglist" :key="res.id" :label="res.plan_name" :name="res.id">
                  ...
                  <div class="planepan" v-if="planopty == 1">
                    ...
                    <template v-for="(rev,index) in planlist">
                      <div class="plfix passcur">
                        ...
                        <p class="fixp1">需完成：
                          <template v-for="(val,index) in rev.phase">
                            <span class="fixval" v-for="(cmt,index) in val.item">{{val.type}} - {{cmt.name}}，</span>
                          </template>
                        </p>
                        <div class="plansum gray_14">
                          <span class="planal" v-for="(rangeval,index) in rev.range">
...
<script>
  ...
  import {CheckAllPlan} from '../api/plan.js'
  ...
  export default {
    ...
    methods: {
      async getCheckAllPlan(){
        ...
        let ret = await CheckAllPlan(this.clid,this.cid);
