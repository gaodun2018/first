<template>
    <div>
        <el-row :gutter="20">
            <el-col :span="10">
                 <el-tree style="max-height: 500px; overflow: auto" :check-strictly="true" :default-expanded-keys="[this.defaultKeys]" :data="this.$store.state.channel.treeData"  empty-text="加载中..."  @node-click="selectNode" :props="defaultProps" node-key="Id" :render-content="renderContent" :highlight-current="true"></el-tree>
            </el-col>
            <el-col :span="14">
                <div class="channel-info">
                    <el-row :gutter="5">
                        ...
                        <el-col :span="16">
                            <el-input size="small" @change="updateName" :value="this.$store.state.channel.currentNode.Name"></el-input>
                        </el-col>
                    </el-row>
                    <el-row :gutter="5" v-if="this.$store.state.channel.currentNode.ChannelNo">
                        <el-col :span="5" :offset="1">
                            <label>渠道编码：</label>
                        </el-col>
                        <el-col :span="6">
                            <label class="c-l-info" >{{this.$store.state.channel.currentNode.ChannelNo}}</label>
                        </el-col>
                    </el-row>
                    <el-row :gutter="5">
                        <el-col :span="6" :offset="6">
                            <div class="c-i-buttons">
                                <el-button v-loading="this.$store.state.channel.loading" style="width: 70px" type="primary" size="small" @click="updateChannelName">保存</el-button>
                            </div>
                        </el-col>
                    </el-row>
                </div>
    ...
    methods: {
        ...
        renderContent(h, { node, data, store }) {
            ...
            let triggerRemoveHandle = (e)=>{
                e.stopPropagation();
                MessageBox.confirm('是否删除ID为 '+ data.Id + ' 的渠道?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$store.dispatch('deleteChannelNode',data.Id);
