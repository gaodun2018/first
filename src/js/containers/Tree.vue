...
export default {
	...
	methods: {
		...
		onTriggerTree(event) { // 显示Tree      
			...
			switch (this.treeType) {
				...
				case 'getArea':
					...
					break;
				case 'label':
					if (this.getUserData.length == 0) { // 标签
						this.onGetLabel();
					}
					break;
				default:
		...
		resetChecked(event) { // 清空选项
			...
			// setTimeout(() => {
			// 	this.$refs.tree.setCheckedKeys(["B63A7628-1188-DF11-B08D-6CF049ECE2C7", "0B3B7628-1188-DF11-B08D-6CF049ECE2C7"]);
			// }, 50);
			this.callback(null);
	...
	mounted() {
		this.$nextTick(function() {
			...
			this.$on('setTree', function() {
				// setTimeout(() => {
				// 	this.$refs.tree.setCheckedKeys([1048]);
				// 	console.log(this);
				// }, 50);
			})
		});
