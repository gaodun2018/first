<template>
    <div>
        <el-row :gutter="20">
            <el-col :span="10">
                 <el-tree style="max-height: 500px; overflow: auto" :check-strictly="true" :default-expanded-keys="[this.defaultKeys]" :data="this.$store.state.SourceDetails.treeData" 
            ...
            <el-col :span="14">
                <div class="channel-info">
                    <el-row :gutter="5">
                        ...
                        <el-col :span="16">
                            <el-input size="small" @change="updateName"  :value="this.$store.state.SourceDetails.currentNode.DetailName"></el-input>
                        </el-col>
                    </el-row>
                    <el-row :gutter="5" v-if="this.$store.state.SourceDetails.currentNode.DetailCode">
                        <el-col :span="5" :offset="1">
                            <label>营销计划编码：</label>
                        </el-col>
                        <el-col :span="6">
                            <label class="c-l-info" >{{this.$store.state.SourceDetails.currentNode.DetailCode}}</label>
                        </el-col>
                    </el-row>
                    <el-row :gutter="5">
                        <el-col :span="6" :offset="6">
                            <div class="c-i-buttons">
                                <el-button  style="width: 70px" v-loading="this.$store.state.channel.loading" type="primary" size="small" @click="updateSourceDetailsName">保存</el-button>
                            </div>
                        </el-col>
                    </el-row>
                </div>
    ...
    methods: {
        ...
        renderContent(h, { node, data, store }) {
            ...
            let triggerRemoveHandle = (e)=>{
                e.stopPropagation();
                MessageBox.confirm('是否删除ID为 '+ data.id + ' 的来源详情?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$store.dispatch('deleteSourceDetailsNode',data.id);
