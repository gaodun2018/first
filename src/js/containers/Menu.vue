<template>
    <div>
        <el-menu :default-active="activeId" :unique-opened="true" :router="false" class="el-menu-vertical-demo" @select="handleSelect">
            <el-submenu :index="item.Url" v-if="item.ChildNavigations&&item.isAuth" v-for="(item,index) in menu" :key="item.NavigationId">
                <template slot="title">
                ...
                <el-menu-item :key="item2.NavigationId" :route="{path:`${item2.Url}`}" v-for="(item2,index2) in item.ChildNavigations" :index="item2.Url" v-if="item2.isAuth">
            ...
            <!-- 没有二级菜单的 -->
            <el-menu-item v-if="!item3.ChildNavigations&&item3.isAuth" :key="item3.NavigationId" :route="{path:`${item3.Url}`}" v-for="(item3,index3) in menu" :index="item3.Url  || Math.random()">
                <svg class="icon" aria-hidden="true">
