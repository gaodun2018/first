    ...
    <div class="module-clues-content clues-manage-style">
        <div class="clues-wrap">
            <div class="clues-layout-left">
                <el-row>
                    <el-col :span="12">
                        <el-button v-if="unlocking('Reservation_Comm_GC')" style="position:relative;top:2px;margin-right: 8px" class="cluesButton-icon" :disabled="cluesGuid.length ? false : true" @click="onCluesButton('KickPool')">
                    ...
                    <el-row>
                        <el-col>
                            <el-radio-group v-model="form.FollowUpState" @change="followStatusChange">
                                ...
                                <el-radio style="color: #666;margin-left: 25px" :label="3">已逾期</el-radio>
                            </el-radio-group>
                            <span style="margin-left: 25px">今日预约沟通线索：
                                <span style="color: #f89f7a; font-size: 12px">{{ReservationCount || 0}}</span>
                            </span>
                            <span style="margin-left: 25px">已跟进：
                                <span style="color: #f89f7a; font-size: 12px">{{FollowedCount || 0}}</span>
                            </span>
                            <span style="margin-left: 25px">已逾期：
                                <span style="color: #f89f7a; font-size: 12px">{{ReservationOverCount || 0}}</span>
                            </span>
                        </el-col>
                    </el-row>
                </div>
                ...
                <div class="clues-table NewCluetable" style="margin-top: 14px;">
                    <el-table ref="multipleTable" border v-loading="loading" :data="SaleNewClueSearchList" :row-class-name="tableRowClassName" tooltip-effect="dark" style="width: 100%;" @selection-change="handleSelectionChange" @sort-change="sortChange">
                        ...
                        <el-table-column prop="LastCommunicateTime" sortable label="末次沟通时间" width="145" style="display:  -webkit-box;margin-left: -4px;color:#1f2d3d">
                        </el-table-column>
                        ...
                        <el-table-column prop="EffectivelyChannelName" label="末次客户来源" width="150px" style="display:  -webkit-box;margin-left: -4px;color:#1f2d3d">
                        </el-table-column>
                        <el-table-column prop="ClueQualityName" label="优质线索类型" width="130px" style="color: #666;">
                        </el-table-column>
                        <el-table-column prop="ClueOwnerName" label="线索所属人" width="120" style="color: #666;">
                        </el-table-column>
                    <el-table-column prop="ClueStatusName" sortable label="线索状态" style="color: #666" width="120">
                    </el-table-column>
                        <el-table-column prop="ProfessionName" label="职业状态" style="color: #666" width="120">
                        </el-table-column>
                        <el-table-column label="所在省市" width="150px" style="color: #666">
                            <template scope="scope">
                        ...
                        <el-table-column fixed="right" align="center" label="操作" width="190px" v-if="unlocking(['Reservation_Comm_Tracking', 'Reservation_Comm_Create', 'Reservation_Comm_Details', 'Reservation_Comm_Audition', 'Reservation_Comm_Shared'])">
                            <template scope="scope">
                                <el-button v-if="unlocking('Reservation_Comm_Tracking')" style="margin-right: 5px" type="text" size="small" @click.stop="onClueTracking(scope.$index, SaleNewClueSearchList)">
                                    跟进
                                </el-button>
                                <span v-if="unlocking('Reservation_Comm_Create')" class="href-right">
                                ...
                                <span v-if="unlocking('Reservation_Comm_Details')" class="href-right">
                                ...
                                <el-dropdown trigger="hover" @command="handleDropdown">
                                    ...
                                    <el-dropdown-menu slot="dropdown">
                                        <el-dropdown-item command="cluesAudition" v-if="unlocking('Reservation_Comm_Audition')">
                                            <a style="color: #48576a" target="_blank" @click="MakeLister(scope.$index,scope.row)">预约试听</a>
                                        </el-dropdown-item>
                                        <el-dropdown-item v-if="unlocking('Reservation_Comm_Shared')" command="cluesShared">共享查询</el-dropdown-item>
                                    </el-dropdown-menu>
                                </el-dropdown>
                            </template>
                        </el-table-column>
...
import { GetReservationCommunicatClueList, GetReservationCommunicatClueStatistics, Distribute, KickCluePool } from '../../api/SaleManage';
    ...
    methods: {
        ...
        tableRowClassName(row, index) { // 选中列表行高亮显示
            for (let i = 0; i < this.multipleSelection.length; i++) {
                if (row.CustomGuid === this.multipleSelection[i].CustomGuid) {
                    return 'positive-row';
                }
            }
            if (row.IsOverdue == 1) {
                return "positive-red-row";
            }
            return '';
        },
        ...
        getRepeatedClueStatistics() {
            let parms = {};
            GetReservationCommunicatClueStatistics(parms).then(ret => {
                if (ret.status === 0) {
                    this.ReservationCount = ret.result.NewClueCount;
                    this.FollowedCount = ret.result.FollowUpCount;
                    this.ReservationOverCount = ret.result.LateBookingCount;
                }
            });
        },
        // 今日新线索列表接口
        getCluleDataList() {
            ...
            GetReservationCommunicatClueList(params).then(ret => {
                if (ret.status === 0) {
