    ...
    <div style="margin:0;padding:0;" v-loading="loading">
        <el-form :model="form" ref="form">
            ...
            <el-form-item :label="form.labelRoomName" label-width="116px" v-show="isliveRoom" class="new-invition-form-item-top">
                <el-input v-model="form.LiveRoomTitle" @change="liveChange" auto-complete="off" size="small" style="width:80%;height:"></el-input>
            </el-form-item>
...
export default {
    ...
    methods: {
        ...
        getRoomList() {
            var userid = this.useModel.UserId;
            if (userid != "" && this.form.CourseType != "" && this.liveBegin != "" && this.form.LIType != "") {
                if (this.form.LIType == '1000470') {
                    ...
                    this.getUserLiveRoomList(roomPrm);
                }
            }
        },
        getUserLiveRoomList(roomPrm) {
        ...
        Makeinvitation() {
            ...
            //更新
            if (vitationNo != undefined && vitationNo != "") {
                if (this.form.LIType == "1000471") {
                    this.form.OldRoomID = "";
                    this.form.IsOld = false;
                }
            ...
            //新增
                ...
                this.loading = true;
                CreateLiveRoom(roomParm).then(ret => {
                    this.loading = false;
                    if (ret.status === 0) {
                        this.MssageBox('创建邀约成功', 'success');
                        this.oncancel();
                    }
                }).catch(() => {
                    this.loading = false;
    ...
    mounted() {
        ...
        var roomPrm = {
            UserID: this.useModel.UserId,
            CourseType: this.form.CourseType,
            StartTime: number2DateTime(new Date(), "yyyy-MM-dd HH:mm:ss"),
            LIType: this.form.LIType
        }
        this.getUserLiveRoomList(roomPrm);
    },
    created() {
        this.useModel = JSON.parse(localStorage.getItem(CRM_USER_INFO));
    }
}
</script>