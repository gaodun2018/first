                ...
                <div class="planebox">
                  <el-col :span="13" class="stu_cou_res_card edplane">
                    <el-card :body-style="{ padding: '0px 20px' }">
                      ...
                      <p class="edp2">需完成：
                        <template v-for="rev in planlist.phase">
                          <span v-for="val in rev.item" :key="val.name">{{rev.type}} - {{val.name}}，</span>
                        </template>
                      </p>
                      <div class="edplbotm">
                        <i></i>
...
export default {
  ...
  methods: {
    ...
    async getPlanProgress(planid){
      ...
      let ret = await this.$http.PlanProgress(this.clid)
    ...
    async getProgressStatics(){
      ...
      let ret = await this.$http.ProgressStatics(this.cid,this.clid)
      if(ret.data.status == 0){
        this.ProgressCm = {
    ...
    async getTopoProgress(planid){
      ...
      let ret = await this.$http.TopoProgress(this.clid)
