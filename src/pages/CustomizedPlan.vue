    ...
    <el-dialog title="新增一个阶段学习计划" class="plandialog" :visible.sync="dialogVisible" @close="resetForm('phaseForm')" top="10%" modal-append-to-body="true">
      ...
      <div class="choose-resourse" v-show="active==1">
        <div class="selected">
          ...
          <span v-for="(item, index) in CheckedList" class="checkeditem" :key="index">{{item}}</span>
        </div>
        <div class="resourcebox">
          <el-checkbox-group v-model="checkList" :min="1" @change="handleCheckedCitiesChange">
            <div v-for="item in gradations" :key="item.id">
              <div class="resourcetit">
                <p>{{item.title}}</p>
              </div>
              <div class="checkbox">
                <el-checkbox :label="item.syllabus.id+'#'+itemk.id" v-for="(itemk,index) in item.syllabus.contents" :key="index">{{itemk.title}}
                </el-checkbox>
              </div>
            </div>
            <div v-if="mocks.length!=0">
              ...
              <div class="checkbox">
                <el-checkbox :label="'0#'+item.mock.id" v-for="(item,index) in mocks" :key="index">{{item.mock.title}}
                </el-checkbox>
              </div>
            </div>
          </el-checkbox-group>
...
//        this.CheckedIdList = CheckedIdList;
      ...
      async getClassPlanDetail(plan_id,tabid){
        let ret = await this.$http.getClassPlanDetail(this.class_id);
      ...
      async deleteClassPlan(){
        let ret = await this.$http.deleteClassPlan('1');
//        console.log(ret);
        /*if(ret.data.status == 0){
      ...
      deletePlanPhase(index, row){
        this.$confirm('此操作将删除该阶段计划, 是否继续?', '提示', {
          ...
          this.$http.deletePlanPhase(row.plan_item_id).then((res) => {
      ...
      async addClassPlan(planName){
        // let ret = await this.$http.addClassPlan({
...
//        console.log(ret);
      ...
      async changePhase(id){
        ...
        if(this.CheckedList.length > 0){
          let ret = await this.$http.changePhase({
  ...
  //          "data[content][]": this.CheckedIdList,
      ...
      async getCourseOutline(){
        ...
        let ret = await this.$http.getCourseOutline(url);
        if (ret.data.status == 0) {
